...
impl Image {
    ...
    pub fn from_buffer(buffer: &[u8]) -> Result<Self> {
        ...
        for i_column in 0..width {
            let offset = reader
                ...
                .with_context(|| format!("Unfinished column {} header", i_column))?
                as isize;
            ensure!(
            ...
            loop {
                let row_start = *source
                    .next()
                    .ok_or_else(|| anyhow::anyhow!("Unfinished column {}", i_column))?
                    as usize;
                if row_start == 255 {
                    break;
                }
                let run_length = *source
                    .next()
                    .ok_or_else(|| anyhow::anyhow!("Missing run length col {}", i_column))?
                    as usize;
                ensure!(
                    row_start + run_length <= height,
                    "Image run too big col {}",
                    i_column
                );
                ...
                for row in row_start..(row_start + run_length) {
                    let pixel = *source
                        .next()
                        .ok_or_else(|| anyhow::anyhow!("Missing pixel col {}", i_column))?;
    ...
    pub fn blit(&mut self, source: &Self, offset: [isize; 2], ignore_transparency: bool) {
        ...
        debug!(
            "Blit {}x{} <- {}x{} +{}x{}",
            self.width, self.height, source.width, source.height, offset[0], offset[1]
        );
        ...
        for dy in 0..copy_height {
            ...
            let src_row =
                &source.pixels[sy * src_pitch + x_start..sy * src_pitch + x_start + copy_width];
            let dest_x = (x_start as isize + offset[0]) as usize;
            let dest_row = &mut self.pixels
                [dest_y * dest_pitch + dest_x..dest_y * dest_pitch + dest_x + copy_width];
