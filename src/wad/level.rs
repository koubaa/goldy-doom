...
impl Level {
    ...
    pub fn adjacent_sectors<'a>(&'a self, sector: &WadSector) -> impl Iterator<Item = &'a WadSector> {
        ...
        self.linedefs.iter().filter_map(move |line| {
            ...
            if left == sector_id {
                self.sectors.get(right as usize)
            } else if right == sector_id {
                self.sectors.get(left as usize)
            } else {
                None
            }
        })
    ...
    pub fn neighbour_heights(&self, of: &WadSector) -> Option<NeighbourHeights> {
        ...
        self.adjacent_sectors(of).fold(None, |heights, sector| {
            ...
            Some(match heights {
                Some(h) => NeighbourHeights {
                    lowest_floor: h.lowest_floor.min(floor),
                    highest_floor: h.highest_floor.max(floor),
                    lowest_ceiling: h.lowest_ceiling.min(ceiling),
                    highest_ceiling: h.highest_ceiling.max(ceiling),
                    next_floor: if floor <= of_floor {
                        h.next_floor
                    } else if let Some(nf) = h.next_floor {
                        Some(nf.min(floor))
                    } else {
                        Some(floor)
                    },
